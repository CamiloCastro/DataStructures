...
public class Punto3 {
    ...
    {
        Temperature count = new Temperature("0K");
        for (int i = 0; i < temperatures.size(); i++) {
            count = count.add(temperatures.get(i));
        }
        double kAverageValue = count.getValue("K") / temperatures.size();
    ...
    public Temperature median(MyDoubleLinkedList<Temperature> temperatures)
    {
        ...
        {
            double value1 = temperatures.get(temperatures.size() / 2).getValue("K");
            double value2 = temperatures.get((temperatures.size() / 2) - 1).getValue("K");
